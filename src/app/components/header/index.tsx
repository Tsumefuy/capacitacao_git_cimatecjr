...
export function Header() {
    ...
    return (
        <header className="absolute left-0 w-full h-20 flex items-center justify-between px-6 md:px-20 py-4 bg-color border-b border-black z-50">
            <Link href="/" className="flex items-center text-2xl">
                ...
                <span className="text-secondary font-normal">Store</span>
            </Link>
            <nav className={`md:flex items-center space-x-8 md:space-x-12 text-secundary text-base font-normal 
                ${openMenu ? "block absolute top-20 right-0 w-full transform transition-all duration-300 ease-in-out bg-color shadow-md p-4" : "hidden"} `}>
                {navLinks.map((link) => (
